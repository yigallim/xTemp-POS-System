...
function Menu({ searchValue, filterValue, searchChange, filterChange }) {
  const filteredFoods = foodCategories.reduce((result, category) => {
    ...
    const categoryFoods = foods.filter(
      (food) =>
        ...
        (filterValue === "" || food.name.toLowerCase().includes(filterValue.toLowerCase()))
    );
  ...
  return (
    <section className="pt-4 md:pt-6">
      <h1 className="text-4xl font-bold text-primary mb-2 mx-3 md:mx-8">Menu</h1>
      ...
      {Object.keys(filteredFoods).map((categoryId) => (
        <div key={categoryId} id={`category-${categoryId}`} className="py-3 px-3 md:py-5 md:px-8">
          <h2 className="text-2xl font-bold mb-2">
            {foodCategories.find((category) => category.id === parseInt(categoryId, 10)).name}
          </h2>
          <ul>
            {filteredFoods[categoryId].map((food) => (
              <li key={food.id} className="flex border-b border-gray-300 py-2 md:py-3 w-full">
                <img
                  src={food.src}
                  alt={food.name + " png"}
                ...
                <div className="pl-3 md:pl-5 flex justify-between w-full">
                  <div>
                    ...
                    <p className="text-gray-400 text-xs md:text-base">{food.description}</p>
                  </div>
